...
if st.checkbox('I agree'):
        ...
        st.write('Please, confirm that you agree')
...
if st.button('Build a portfolio'):
        ...
        # Efficient Frontier
        rets = [fc.portfolio_return(w, er) for w in weights]
        vols = [fc.portfolio_vol(w, cov) for w in weights]
        ef = pd.DataFrame({
                "Returns": rets,
                "Volatility": vols
        })
        fig, ax = plt.subplots()
        ax.plot(ef["Volatility"], ef["Returns"], '.-', linewidth=1, markersize=12)
        n = er.shape[0]
        w_ew = np.repeat(1 / n, n)
        # print('Weights in EW', w_ew)
        r_ew = fc.portfolio_return(w_ew, er)
        vol_ew = fc.portfolio_vol(w_ew, cov)
        # display EW
        ax.plot([vol_ew], [r_ew], color='goldenrod', marker='o', markersize=10, label='Equally Weighted')

        w_gmv = fc.gmv(cov)
        # print('Weights in GMV', list(w_gmv))
        r_gmv = fc.portfolio_return(w_gmv, er)
        vol_gmv = fc.portfolio_vol(w_gmv, cov)
        # display GMV
        ...
        r_msr = fc.portfolio_return(w_msr, er)
        vol_msr = fc.portfolio_vol(w_msr, cov)
        ...
        rets_maxdd = fc.bt_mix(pd.DataFrame(gmv_portfolio), rets_cash, allocator=fc.drawdown_allocator, maxdd=.25, m=3)
